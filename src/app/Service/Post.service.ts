...
export class PostService {
    ...
    createPost(token: string, post: Post, file: File): Observable<Post> {
        const formData: FormData = new FormData();
        formData.append('post', JSON.stringify(post));
        formData.append('file', file);

        return this.http.post<Post>(`${this.apiUrl}/create`, formData, { headers: this.getHeaders(token) });
    }
    ...
    getBestPost(): Observable<Post[]> {
        return this.http.get<Post[]>(`${this.apiUrl}/getbest`);
    }
